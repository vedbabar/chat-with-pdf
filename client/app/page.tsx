...
import { Menu, Plus, MessageCircle, FileText, Edit2, Check, X, Loader2, ChevronLeft, ChevronRight, Sun, Moon, BookOpen, PanelLeftClose, PanelLeft } from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <div className="h-screen flex bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-950 dark:to-slate-900 transition-colors duration-300">
      ...
      <div className={`${sidebarOpen ? 'w-80' : 'w-0'} transition-all duration-300 overflow-hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-r border-slate-200/50 dark:border-slate-700/50 flex flex-col shadow-2xl relative`}>
        {/* Sidebar Content */}
        ...
        {/* Collapse Button - Always Visible on Sidebar */}
        <button
          onClick={() => setSidebarOpen(false)}
          className="absolute -right-3 top-1/2 z-20 transform -translate-y-1/2 bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-700 rounded-full shadow-lg p-2 hover:bg-slate-50 dark:hover:bg-slate-700 transition transform hover:scale-110"
      ...
      {!sidebarOpen && (
        <button
          ...
          className="fixed left-4 top-1/2 z-20 transform -translate-y-1/2 bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-700 rounded-full shadow-2xl p-3 hover:bg-slate-50 dark:hover:bg-slate-700 transition transform hover:scale-110 animate-fade-in"
          ...
          <div className="lg:hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-700/50 px-4 py-2 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                className="h-8 w-8 text-slate-600 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 rounded-xl"
